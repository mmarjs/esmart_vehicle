  ...
  <div v-if="!loading">
    <table class="flex-1 table-auto w-full mb-4 rounded-xl overflow-hidden">
      <thead class="bg-gray-500">
        <tr>
          <th scope="col" class="border-r-1 border-gray w-[200px]">
          ...
          >
            <td class="px-4 border-r-1 border-gray">
              ...
              <span class="ml-5 mr-4">{{
                format(new Date(notif.time), "hh:mm:ss")
              }}</span>
            </td>
...
import { format, fromUnixTime, addSeconds, set, addHours } from "date-fns";
...
const geofences = computed(() => geofenceStore.geofencesNotif);
...
const init = async () => {
  ...
  if (filters.timestamp && !filters.date.start) {
    filters.date = {
      start: format(new Date(fromUnixTime(filters.timestamp)), "yyyy-MM-dd"),
    };
  }
